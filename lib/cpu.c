...
void log_cpu_state(CPU *cpu) {
    uint8_t b0 = mem_read(cpu->pc + 0);
    uint8_t b1 = mem_read(cpu->pc + 1);
    uint8_t b2 = mem_read(cpu->pc + 2);
    uint8_t b3 = mem_read(cpu->pc + 3);
...
- sets the program counter to the address of the interrupt
*/
static void ack_interrupts(CPU *cpu) {
    uint8_t pending =
...
for example, if pending is 0b00001000, id = 3 */
    ...
    ret--;
    cpu->sp--;
    mem_write(cpu->sp, ret & 0x00FF); /* push the low byte */
    cpu->sp--;
    mem_write(cpu->sp, ret >> 8); /* push the high byte */
